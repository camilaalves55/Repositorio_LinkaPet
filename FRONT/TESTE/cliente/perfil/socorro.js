    ...
    try {
        ...
        if (clienteData.success) {
            ...
            const getImageUrl = (fileName) => {
                return `http://localhost:3005/upload/${fileName}?t=${new Date().getTime()}`;
            };
            ...
            document.getElementById('foto_perfil').src = getImageUrl(fotoPerfil);
            ...
            document.getElementById('okBtn').addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (result.success) {
                        document.getElementById('foto_perfil').src = getImageUrl(result.data.foto_perfil);
                        Swal.fire({
                            title: 'Sucesso!',
            ...
            if (petsResult.success) {
                ...
                petsResult.data.forEach(pet => {
                    ...
                    img.src = getImageUrl(pet.imagem); // Usar a função para obter a URL
...
//                                         imgPet.src = getImageUrl(result.data.imagem);
//                                         div_cards.appendChild(imgPet);
//                                     } else {
//                                         Swal.fire({
//                                             title: "Erro",
//                                             text: result.message,
...
//                         // Adicione aqui o FileReader para mostrar a imagem carregada
//                         const reader = new FileReader();
//                         reader.onload = async function (event) {
//                             const img = document.createElement('img');
//                             img.src = event.target.result; // Usando o resultado do leitor como src
//                             document.body.appendChild(img); // Adiciona a imagem ao body para visualização (opcional)
...
//                             // Mostra a imagem carregada na interface, se necessário
//                             // Aqui você pode definir onde a imagem deve ser exibida
//                             // div_cards.appendChild(img);  // Por exemplo, adicionar a imagem à div_cards
...
//                                         // Se necessário, você pode adicionar o novo pet à visualização
//                                         const imgPet = document.createElement('img');
//                                         imgPet.src = getImageUrl(result.data.imagem); // Usando a função para obter a URL da imagem
//                                         div_cards.appendChild(imgPet); // Adiciona a imagem à div_cards, se necessário
